...
class BinaryASTSerializer {
	...
	private static function deserialize(bytes:Bytes):Dynamic {
	...
	static function serializeValue(val:Dynamic, out:BytesOutput, stringMap:Map<String, Int>) {
		...
		if (Std.isOfType(val, BinaryResourceRefHolder)
			|| (Reflect.isObject(val) && Reflect.hasField(val, "key") && !Reflect.hasField(val, "def"))) {
			out.writeByte(10);
