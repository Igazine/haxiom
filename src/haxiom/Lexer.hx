...
class Lexer {
	...
	function new(input:String, ?file:String, ?flags:Map<String, Bool>) {
		this.input = input;
	...
	function tokenize():Array<Token> {
		...
		while (pos < input.length) {
			...
			switch (char) {
				...
				case "[":
					add(tokens, TBracketOpen);
				case "]":
					add(tokens, TBracketClose);
				case "{":
					add(tokens, TBraceOpen);
				case "}":
					add(tokens, TBraceClose);
				case ",":
					add(tokens, TComma);
				case ".":
					if (peek(1) == "." && peek(2) == ".")
						...
						add(tokens, TDot);
				case ";":
					add(tokens, TSemicolon);
				case "?":
					if (peek(1) == ".")
						add(tokens, TQuestionDot, 2);
					else if (peek(1) == "?")
						add(tokens, TDoubleQuestion, 2);
					else
						add(tokens, TQuestion);
				case ":":
					add(tokens, TColon);
				case "@":
					add(tokens, TAt);
				...
				case "+":
					if (peek(1) == "+")
						add(tokens, TIncrement, 2);
					else if (peek(1) == "=")
						add(tokens, TPlusAssign, 2);
					else
						add(tokens, TPlus);
				case "-":
					if (peek(1) == "-")
						add(tokens, TDecrement, 2);
					else if (peek(1) == "=")
						add(tokens, TMinusAssign, 2);
					else if (peek(1) == ">")
						add(tokens, TArrow, 2);
					else
						add(tokens, TMinus);
				case "*":
					if (peek(1) == "=")
						add(tokens, TStarAssign, 2);
					else
						add(tokens, TStar);
				case "/":
					if (peek(1) == "=")
						add(tokens, TSlashAssign, 2);
					else
						add(tokens, TSlash);
				case "%":
					if (peek(1) == "=")
						add(tokens, TPercentAssign, 2);
					else
						add(tokens, TPercent);
				...
				case "=":
					if (peek(1) == "=")
						...
						add(tokens, TMapArrow, 2);
					else
						add(tokens, TAssign);
				case "!":
					if (peek(1) == "=")
						add(tokens, TNotEqual, 2);
					else
						add(tokens, TNot);

				case "<":
					if (peek(1) == "=")
						add(tokens, TLessEqual, 2);
					else if (peek(1) == "<")
						add(tokens, TShiftLeft, 2);
					else
						add(tokens, TLess);
				case ">":
					if (peek(1) == "=")
						add(tokens, TGreaterEqual, 2);
					else if (peek(1) == ">" && peek(2) == ">")
						add(tokens, TUnsignedShiftRight, 3);
					else if (peek(1) == ">")
						add(tokens, TShiftRight, 2);
					else
						add(tokens, TGreater);
