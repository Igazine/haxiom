...
class Parser {
	...
	function registerType(name:String, pos:Pos) {
		if (definedTypes.exists(name)) {
			...
			throw new CompileException("Redefinition of class " + name + " (previously defined at line " + prev.line + ", col " + prev.col + ")", pos.line,
				pos.col, file);
		}
		definedTypes.set(name, pos);
	}

	function new(tokens:Array<Token>, ?file:String) {
		this.tokens = tokens;
	...
	function parseStatement():Expr {
		...
		switch (t.def) {
			...
			case TImport:
				if (meta != null)
					throw new CompileException("Metadata cannot be attached to an import declaration", t.pos.line, t.pos.col, file);
				next();
				var path = [];
				if (match(TStar)) {
					path.push("*");
			...
			case TUsing:
				if (meta != null)
					throw new CompileException("Metadata cannot be attached to a using declaration", t.pos.line, t.pos.col, file);
				next();
				var path = [];
				path.push(expectIdent());
				while (match(TDot)) {
	...
	function parseShift():Expr {
		...
		while (is(TShiftLeft) || is(TShiftRight) || is(TUnsignedShiftRight)) {
			...
			if (match(TShiftLeft))
				op = "<<";
			else if (match(TShiftRight))
				op = ">>";
			else if (match(TUnsignedShiftRight))
				op = ">>>";
	...
	function parseBitwise():Expr {
		...
		while (is(TBitAnd) || is(TBitOr) || is(TBitXor)) {
			...
			if (match(TBitAnd))
				op = "&";
			else if (match(TBitOr))
				op = "|";
			else if (match(TBitXor))
				op = "^";
	...
	function isMapComprehension(expr:Expr):Bool {
		...
		switch (expr.def) {
			case EBlock(exprs):
				if (exprs.length > 0)
					return isMapComprehension(exprs[exprs.length - 1]);
				return false;
			case EIf(_, e1, e2):
			...
			case ESwitch(_, cases, defExpr):
				for (c in cases) {
					if (isMapComprehension(c.expr))
						return true;
				}
				return defExpr != null && isMapComprehension(defExpr);
			case ETry(tryExpr, catches):
				if (isMapComprehension(tryExpr))
					return true;
				for (c in catches) {
					if (isMapComprehension(c.body))
						return true;
				}
				return false;
			case EBinop("=>", _, _):
	...
	function transformComprehension(expr:Expr, compName:String, isMap:Bool):Expr {
		...
		switch (expr.def) {
			...
			case ESwitch(switchExpr, cases, defExpr):
				var newCases = [
					for (c in cases) {values: c.values, guard: c.guard, expr: transformComprehension(c.expr, compName, isMap)}
				];
			...
			case ETry(tryExpr, catches):
				...
				var newCatches = [
					for (c in catches) {
						...
						type: c.type,
						guard: c.guard,
						body: transformComprehension(c.body, compName, isMap)
					}
				];
