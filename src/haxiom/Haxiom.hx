...
class Haxiom {
	...
	public function resolveField(target:Dynamic, field:String):Dynamic {
		if (interp.disposed)
			throw "Cannot resolve a guest field on a disposed Haxiom engine instance";
		return interp.evalField(target, field, interp.globals, {line: 1, col: 1, file: "host"});
	}

	private function invokeProxyMethod(target:Dynamic, field:String, args:Array<Dynamic>, argTypes:Array<ProxyBoundaryType>, returnType:ProxyBoundaryType):Dynamic {
		if (interp.disposed)
	...
	private function readProxyField(target:Dynamic, field:String, type:ProxyBoundaryType):Dynamic {
		return ProxyBoundary.convert(resolveField(target, field), type, '$field property read');
	}

	private function writeProxyField(target:Dynamic, field:String, value:Dynamic, type:ProxyBoundaryType):Dynamic {
		if (interp.disposed)
	...
	public function setField(target:Dynamic, field:String, value:Dynamic):Void {
		if (interp.disposed)
			throw "Cannot set a guest field on a disposed Haxiom engine instance";
	...
	/**
	 * Compile-time macro to construct a guest class and adapt it to a host-defined interface proxy.
	 * 
	...
	public macro function construct<T>(ethis:Expr, className:Expr, ?targetInterfaceExpr:Expr):haxe.macro.Expr.ExprOf<T> {
		...
		var interfaceClass:haxe.macro.Type.ClassType = null;
		...
		if (interfaceClass == null) {
			Context.error("construct() only supports host interface types", className.pos);
		}
		function rejectGenericInterfaces(itf:haxe.macro.Type.ClassType):Void {
			if (itf.params.length > 0)
				Context.error("construct() does not yet support generic host interfaces", className.pos);
			for (parent in itf.interfaces)
				rejectGenericInterfaces(parent.t.get());
		}
		rejectGenericInterfaces(interfaceClass);

		var allFields:Map<String, haxe.macro.Type.ClassField> = new Map();
		function collectInterfaceFields(itf:haxe.macro.Type.ClassType):Void {
			for (field in itf.fields.get())
				allFields.set(field.name, field);
			for (parent in itf.interfaces)
				collectInterfaceFields(parent.t.get());
		}
		collectInterfaceFields(interfaceClass);
		...
		var methodContracts:Array<Expr> = [];
		var fieldContracts:Array<Expr> = [];
		for (field in allFields) {
			...
			switch (field.kind) {
				case FMethod(_):
					switch (Context.follow(field.type)) {
						case TFun(args, ret):
							var argTypes = [for (arg in args) macro $v{haxe.macro.TypeTools.toString(Context.follow(arg.t))}];
							methodContracts.push(macro {
								name: $v{fieldName},
								argTypes: $a{argTypes},
								returnType: $v{haxe.macro.TypeTools.toString(Context.follow(ret))}
							});
						default:
					}
				case FVar(read, write):
					...
					fieldContracts.push(macro {
						name: $v{fieldName},
						typeName: $v{haxe.macro.TypeTools.toString(Context.follow(field.type))},
						readable: $v{readable},
						writable: $v{writable}
					});
			}
		...
		if (expectedTypeStr != null) {
			...
			registerExprs.push(macro @:privateAccess Haxiom.registerInterface($ethis, $v{expectedTypeStr}, $hostInterfaceIdentity));
		}
		...
		return macro {
			$b{registerExprs};
			(cast @:privateAccess Haxiom.constructHelper($ethis, $className, $v{expectedTypeStr}, $a{methodContracts}, $a{fieldContracts}, function(h, guest) {
	 ...
	 */
	private static function registerInterface(haxiom:Haxiom, name:String, hostIdentity:Dynamic):Void {
	 ...
	 * @param interfaceName Fully-qualified host interface the guest class must implement.
	 * @param methods Host interface method contract generated by the construct macro.
	 * @param fields Host interface field/property contract generated by the construct macro.
	 ...
	 */
	private static function constructHelper(haxiom:Haxiom, className:String, interfaceName:String,
		methods:Array<{name:String, argTypes:Array<String>, returnType:String}>,
		fields:Array<{name:String, typeName:String, readable:Bool, writable:Bool}>, factory:(Haxiom, Dynamic) -> Dynamic):Dynamic {
		if (haxiom == null)
			throw "construct() requires a Haxiom instance";
		var guestClass = haxiom.interp.validateGuestInterface(className, interfaceName, methods, fields);
		var guestInst = haxiom.interp.constructGuestClass(guestClass);
		if (guestInst == null)
			throw 'Failed to construct guest class $className';
