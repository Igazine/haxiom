...
class InlineCacheEntry {
	var lastClass:Dynamic = null;
	var lastObject:Dynamic = null;
	var cachedValue:Dynamic = null;
	var isNormalField:Bool = false;
	var isMethod:Bool = false;
	var isProperty:Bool = false;
	var isNativeProperty:Bool = false;
	var fieldName:String = null;

	// For methods:
	var cachedMethodDef:Dynamic = null;

	// For getters / setters:
	var getterMethod:Dynamic = null;
	var setterMethod:Dynamic = null;

	// Megamorphic fallback:
	var isMegamorphic:Bool = false;

	// Linked list next pointer:
	var next:InlineCacheEntry = null;
...
@:allow(haxiom)
class BytecodeChunk {
	var inlineCaches:Map<Int, InlineCacheEntry> = new Map();
	...
	function getBytes(?key:haxiom.HXBCKey):haxe.io.Bytes {
		return haxiom.Serializer.serializeBytecode(this, key);
	}

	static function fromBytes(bytes:haxe.io.Bytes, ?key:haxiom.HXBCKey):BytecodeChunk {
...
class VMCallFrame {
	...
	function new(chunk:BytecodeChunk, ip:Int, scope:Scope, ?methodName:String = "") {
		this.chunk = chunk;
		this.ip = ip;
		this.scope = scope;
		...
		try {
			while (fiber == null || !fiber.isSuspended) {
				if (interp.maxInstructions > 0 && ++interp.instructionsCount > interp.maxInstructions) {
					...
					var lineVal = cp.line;
					var colVal = cp.col;
					var locationStr = 'Runtime Error: Instruction limit exceeded (${interp.maxInstructions} ops) at '
						+ fileInfo
						+ ':'
						+ lineVal
						+ ':'
					...
					var vmCallStack = [
						for (f in callFrames)
							{method: f.methodName != null ? f.methodName : "anonymous", pos: cp}
					];
					throw new haxiom.ScriptException("Instruction limit exceeded (possible infinite loop)", vmCallStack, locationStr, lineVal, colVal,
						fileInfo);
				}
				try {
					...
					switch (op) {
								...
								if (cache != null && !cache.isMegamorphic && classKey != null) {
									...
									while (curr != null) {
										if (curr.lastClass == classKey) {
											if (curr.isNormalField) {
												if (Std.isOfType(obj, HaxiomInstance)) {
													if (!(cast obj : HaxiomInstance).fields.exists(fieldName))
														interp.trackMemory(1);
						...
						case OP_NEW_MAP:
							...
							if (size == 0) {
								...
								var allInt = true;
								for (kv in evaluated) {
									if (!Std.isOfType(kv.key, String))
										allString = false;
									if (!Std.isOfType(kv.key, Int))
										allInt = false;
								}
								if (allString) {
...
class HaxiomSuperInstance {
	var inst:HaxiomInstance;
	var interp:Interp;
	var scope:Scope;

	function new(inst:HaxiomInstance, interp:Interp, scope:Scope) {
		this.inst = inst;
		this.interp = interp;
		this.scope = scope;
	}

	function callConstructor(args:Array<Dynamic>):Dynamic {
